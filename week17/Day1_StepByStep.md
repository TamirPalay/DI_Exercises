...
### Way 4 — the visual inspector ⭐ (the best one — zero code, great on a projector)

This is the most convincing proof for students: no client script at all, just a
browser UI with **Run** buttons. **From inside this folder**, run:
...
- **The `--` matters.** Without it, the Inspector ignores your command and tries
  to launch its own demo server (`mcp-server-everything`) — you'll see repeated
  `ENOENT` errors. `--` means "everything after this is the server to launch."
- **Run it from the `Day1-Build-a-Server` folder** (or give the full path in the
  command), or you'll get `can't open file … day1_server_full.py`.

A browser UI opens. Click the **toggle** next to the server so it flips from
*Disconnected* to a green **Connected** — the Inspector launches your server in
**STDIO mode** (so you do *not* also run it in Terminal 1; it starts it for you,
and no port 8000 is involved here). Then open the **Tools** tab: you'll see all
your tools (`read_file`, `get_weather`, `wikipedia_summary`, `web_search`), the
resource, and the prompt — each with a **Run** button. That tool listing is
literally Step 1 of tomorrow's loop ("discover").

Try each tool live:

- `read_file` → `notes.txt` → returns the file's text.
- `get_weather` → `Tel Aviv` → returns the live temperature.
- `wikipedia_summary` → a **page title** like `Ivory Coast` — **not a question**.
  Passing "what is the capital of Ivory Coast?" returns a 404 (there's no page by
  that name); this is a great moment to show why the docstring says "a page
  title, NOT a full question."
- `web_search` → a **natural query** like `EUR to USD exchange rate` → returns
  live web results. (It returns search snippets, not a parsed answer — turning
  those into a final answer is the LLM's job on Day 2.)
